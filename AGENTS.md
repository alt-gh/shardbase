...
- Require `aliases`, `id`, and `tags` on every Core, Shard, and Pebble in addition to the five structural fields. These are reserved common note fields governed by System Specification Section 8.6, not lineage fields. Default each to a blank YAML value; preserve populated values. Populated aliases and tags are lists of non-empty strings; an ID is a scalar string and is never automatically generated. This requirement does not apply to documentation, manifests, Agent resources, Views, attachments, or Inbox captures.
