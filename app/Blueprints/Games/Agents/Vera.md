...
4. Use the five universal structural fields and applicable documented semantic fields. Core `core` self-references and `parent_note` is empty. Descendants resolve to the same-database root Core and an existing Core or Shard parent; Pebbles cannot parent notes. Structural `status` is not personal play progress. Omit unknown optional metadata.
