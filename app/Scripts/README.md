...
The validator performs read-only structural checks against the current [System Specification](../Docs/Shard%20System%20Specification.md). A successful run means the implemented checks passed; it is not proof of full database-semantic conformance or Foundation completion.
...
- Check manifest fields and scalar/list shapes, integer `manifest_version: 1`, required section presence, collection declarations, root `Attachments/`, and `Views/`. Unsupported manifest versions stop inspection of that database's contents.
...
This update implements existing architectural rules and corrects false acceptance and crashes; it does not change the System Specification or `manifest_version`. Previously accepted invalid state can now produce diagnostics. Nothing is automatically normalized: existing live state must be reviewed under its applicable contract before any separately authorized preservation-oriented correction. The validator is not a general migration engine.
